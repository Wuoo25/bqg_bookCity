...
<script>
    ...
    function getTypeList(pageNo){
        ...
        $.post('type/getList',{'pageNo':pageNo},function (data){
            ...
            if (data.code == 1) {
                var pageDate=data.data;
                var list=pageDate.list;//获取的是pagedate
                var html=`<table>
                          <tr><td>类型名称</td><td>介绍</td><td>创建时间</td><td>修改时间</td></tr>`;
                //拼接table表格
                //item是遍历出来的元素
                ...
                for (var i = 1; i <= pages; i++) {
                    ...
                    if (pageData.pageNo == i) {
                        html+='<button style="background-color:grey">'+i+'</button>';
                    }else{
